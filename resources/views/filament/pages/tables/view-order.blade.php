<div class="px-4 py-3 bg-gray-100 dark:bg-gray-900 rounded-lg">
    ...
    @endsection
    <div class="p-5 mb-5 block bg-white dark:bg-gray-900 shadow-md border rounded-3xl w-full" >
        <div class="block w-full">
            <style>.ps-2.pe-4.block.w-full{display:none}</style>
             <div class="md:flex items-center justify-start gap-8 text-gray-900 dark:text-white w-full py-5 px-6">
                <div class="flex items-center gap-2 font-semibold text-base lg:text-xl">
                    {{-- <img class="w-10 h-10 rounded-full" src="http://localhost:1313/images/products/imac.png" alt=" avatar"> --}}
                    {{trans('dash.order_id')}}
                    <a href="jafascript:void" class="underline">#{{$order->id}}  {{$order->name ?? ''}}</a>
                </div>
                <span><strong> {{trans('dash.Expected_Delivery_Date')}} </strong> {{$order->expected_delivery_date}}</span>
                {{-- <span class="rounded-3xl  p-1 px-3 text-sm text-white ltr:float-right rtl:float-left md:float-none @if($order->payment_status == "Paid") bg-green-500 @else bg-red-500 @endif"   >{{$order->payment_status}}</span> --}}
                <x-filament::badge size="xl"   :color="$order->payment_status =='Paid' ? 'success' : 'danger'">
        ...
        <div class="bg-white pb-4 antialiased dark:bg-gray-900 ">
            <form action="#" class="mx-auto  px-4 2xl:px-0">
                    ...
                    <table class="w-full text-left font-medium text-gray-900 dark:text-white md:table-fixed">
                      <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        ...
                        <tr>
                          ...
                          <td class="p-4 text-right text-base font-bold text-gray-900 dark:text-white">{{$order->currency->symbol}} {{$product->price}}</td>
                        </tr>
                        @endforeach
                    ...
                    <div class="space-y-4">
                      <div class="space-y-2">
                        <dl class="flex items-center justify-between gap-4">
                          ...
                          <dd class="text-base font-medium text-gray-900 dark:text-white">{{$order->currency->symbol}} {{$total}}</dd>
                        </dl>
                        <dl class="flex items-center justify-between gap-4">
          ...
          <div id="billingInformationModal{{$order->id}}" tabindex="-1" aria-hidden="true" class="antialiased fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-auto w-full max-h-full items-center justify-center overflow-y-auto overflow-x-hidden antialiased md:inset-0">
            <div class="relative max-h-auto w-full max-h-full max-w-lg p-4">
              <!-- Modal content -->
              <div class="relative rounded-lg bg-white shadow dark:bg-gray-800">
                <!-- Modal header -->
                <div class="flex items-center justify-between rounded-t border-b border-gray-200 p-4 dark:border-gray-700 md:p-5">
                  <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{trans('dash.billing_delivery')}}</h3>
                  <button type="button" class="ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="billingInformationModal">
                    ...
                    <span class="sr-only">{{trans('dash.close')}}</span>
                  </button>
                </div>
                <!-- Modal body -->
                <form class="p-4 md:p-5">
                  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-5">
                    ...
                    <div class="sm:col-span-2">
                      <div class="mb-2 flex items-center gap-1">
                        <label for="saved-address-modal" class="block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.saved_addresses')}} </label>
                    ...
                    <div>
                      <label for="first_name_billing_modal" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.first_name')}}* </label>
                      <input type="text" wire:model="shipping_address.{{$recordKey}}.first_name" id="first_name_billing_modal" value="{{$order->shippingAddress->first_name}}" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Enter your first name" required />
                    </div>
            
                    <div>
                      <label for="last_name_billing_modal" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.last_name')}}* </label>
                      <input type="text" wire:model="shipping_address.{{$recordKey}}.last_name" value="{{$order->shippingAddress->last_name}}" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Enter your last name" required />
                    </div>
            
                    <div class="sm:col-span-2">
                      <label for="phone-input_billing_modal" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.phone_number')}}* </label>
                      <div class="flex items-center">
                        ...
                        <div class="relative w-full">
                          <input type="text" value="{{$order->shippingAddress->phone_number}}" id="phone-input" class="z-20 block w-full rounded-e-lg border border-s-0 border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:border-s-gray-700  dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500"  placeholder="123-456-7890" required />
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <div class="mb-2 flex items-center gap-2">
                        <label for="select_country_input_billing_modal" class="block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.country')}}* </label>
                      </div>
                      <input type="text" id="select_country_input_billing_modal" value="{{$order->shippingAddress->country}}" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Enter your country" required />
                    ...
                    <div>
                      <div class="mb-2 flex items-center gap-2">
                        <label for="select_city_input_billing_modal" class="block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.city')}}* </label>
                      </div>
                      <input type="text" id="select_city_input_billing_modal" value="{{$order->shippingAddress->city}}" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Enter your city" required />
                    ...
                    <div>
                      <div class="mb-2 flex items-center gap-2">
                        <label for="select_city_input_billing_modal" class="block text-sm font-medium text-gray-900 dark:text-white"> {{trans('dash.zip_code')}}* </label>
                      </div>
                      <input type="text" id="zip_code" value="{{$order->shippingAddress->zip_code}}" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" placeholder="Enter your city" required />
                    </div>
                  ...
                  <div class="border-t border-gray-200 pt-4 dark:border-gray-700 md:pt-5">
                    <button wire:submit.prevent="updateShippingAddress()" class="me-2 inline-flex items-center rounded-lg bg-primary-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">{{trans('dash.save')}}</button>
                    <button type="button"  class="me-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700">{{trans('dash.cancel')}}</button>
                  </div>
                </form>
              </div>
            </div>
